...
function applyOutcome(
  ...
  if (wasChecking || previousStatus !== status) {
    if (status === 'cached') {
      logEvent(
        'WARN',
        wasChecking
      ...
      logEvent(
        'ERROR',
        wasChecking
          ? 'API unreachable — no cached data available'
          : 'API connection lost — no cached data available',
      );
    }
  } else if (failures > 0 && failures % 10 === 0) {
    logEvent('DEBUG', `Health probe still failing (${failures} consecutive): ${evidence.error ?? 'unknown error'}`);
  }
}
