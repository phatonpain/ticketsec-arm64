...
describe('formatRelativeTime — invalid input (EXPOSED defect)', () => {
  ...
  it.fails(
    'EXPOSED-01: invalid Date returns "NaNd ago" — should return "—" (formatRelativeTime.ts:3-11)',
    () => {
  ...
  it.fails(
    'EXPOSED-01: unparsable date string returns "NaNd ago" — should return "—"',
    () => {
      expect(formatRelativeTime(new Date('not a date'))).toBe('—');
    },
  );

  it('documents the actual broken output for invalid input', () => {
    // Pins the current (defective) behavior so any fix updates this test.
    expect(formatRelativeTime(new Date(Number.NaN))).toBe('NaNd ago');
  });
});
