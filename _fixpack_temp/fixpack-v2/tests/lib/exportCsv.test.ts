...
const REAL_HEADER = 'ID,Subject,Category,Confidence,Status,Assigned To,Created At';
...
describe('exportTicketsToCsv — structure (real format)', () => {
  ...
  it('serializes a plain ticket unquoted; confidence as fixed-1 number; ISO time', async () => {
    ...
    const lines = (await csvOf()).split('\n');
    expect(lines[1]).toBe(
      `TKT-8471,Suspicious email asking for bank credentials,Phishing,96.0,Resolved,Auto,${createdAt.toISOString()}`,
    );
  });
