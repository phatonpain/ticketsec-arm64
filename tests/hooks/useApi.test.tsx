...
/** Current store state via a mounted hook (reads store without remounting). */
...
describe('checkHealth — offline honesty (no cache yet)', () => {
  it('failed probe → status "offline" (NOT "cached"), diagnostics populated', async () => {
    const { result } = renderHook(() => useApi());
    // Let the mount-time probe settle before driving a manual checkHealth.
    await vi.waitFor(() => expect(result.current.checking).toBe(false));
    ...
    expect(result.current.status).toBe('offline'); // hasAnyCache() === false → offline
    expect(result.current.diagnostics.lastProbe).toBeInstanceOf(Date);
    expect(result.current.diagnostics.endpoints).toHaveLength(3);
    expect(result.current.diagnostics.lastError).toBe('TypeError');
  });

  it('concurrent checkHealth calls are outcome-guarded (only the latest result applies)', async () => {
    await Promise.all([checkHealth(), checkHealth(), checkHealth()]);
    // The inFlight abort/supersede guard still issues fetch calls from the
    // discarded probe rounds before they are aborted, so 9 calls are observed
    // (3 rounds × 3 endpoints) but only one result is applied.
    expect(fetchMock.calls).toHaveLength(9);
    expect(fetchMock.calls).toContain(`${BASE}/health`);
    expect(fetchMock.calls).toContain(`${BASE}/`);
    expect(fetchMock.calls).toContain(`${BASE}/docs`);
  });
});
